...
def main(args=[]):
    ...
    if  len(args) > 0 :
        ...
        scale = int(args[1])
        if len(args)>2:
            dataset = args[2].split('+')
            if len(args)>3:
                if args[3] == 'Y' or 'y':
                    test_Y = True
                elif args[3] == 'RGB' or 'rgb':
                    test_Y = False
            else:
                test_Y = True
        else:
    ...
    for m in range(len(model)):
        ...
        for n in range(len(dataset)):
            ...
            get_aver[1].append('{:.4f}'.format(sum(PSNR_all) / len(PSNR_all)))
            get_aver[3].append('{:.5f}'.format(sum(SSIM_all) / len(SSIM_all)))
        print(model[m],)
        for i in range(len(get_aver)):
            print(get_aver[i])
    ...
    print('*'*20)
    print(dataset)
    for j in range(len(all_mem)):
        print('*'*20)
        for i in range(len(all_mem[j])):
            print(all_mem[j][i])
