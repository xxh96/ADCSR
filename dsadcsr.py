...
def abandon():#Set ''SKIP AND TAIL'  trainablr = False 
    return ['skip','tail']
...
class FFSC(nn.Module):
    def __init__(
        ...
        if scale == 16 :
            ...
            self.tail_23 = wn(nn.Conv2d(mido_feats, 12, 3, padding=3//2, dilation=1))
            self.tail_25 = wn(nn.Conv2d(mido_feats, 12, 5, padding=5//2, dilation=1))
            self.tail_27 = wn(nn.Conv2d(mido_feats, 12, 7, padding=7//2, dilation=1))
            self.tail_29 = wn(nn.Conv2d(mido_feats, 12, 9, padding=9//2, dilation=1))
        
            self.TFF2 = nn.Sequential(*[
                wn(nn.Conv2d(last_feats*4, last_feats, 1, padding=0, stride=1),),
            ])
            self.pixelshuffle_0 = nn.PixelShuffle(4)
            self.pixelshuffle_1 = nn.PixelShuffle(2)
    ...
    def forward(self, x):
        if self.scale == 16:
            ...
            outmix1 = []
            outmix1.append(self.pixelshuffle_0(self.tail_13(x)))
            outmix1.append(self.pixelshuffle_0(self.tail_15(x)))
            outmix1.append(self.pixelshuffle_0(self.tail_17(x)))
            outmix1.append(self.pixelshuffle_0(self.tail_19(x)))
            ...
            outmix2 = []
            outmix2.append(self.pixelshuffle_1(self.tail_23(x)))
            outmix2.append(self.pixelshuffle_1(self.tail_25(x)))
            outmix2.append(self.pixelshuffle_1(self.tail_27(x)))
            outmix2.append(self.pixelshuffle_1(self.tail_29(x)))
...
class DSADCSR(nn.Module):#ADC16_12
